SECTIONS
{
	...
	# PS4 rtld rejects the OELF if create-eboot's RELRO-style segment starts
	# mid-page. lld 14 puts .got.plt in a separate RW LOAD, so force it to a
	# page boundary before create-eboot converts the program headers.
	.got.plt : ALIGN(0x4000) {
		*(.got.plt)
	}

	.data.sce_process_param : ALIGN(0x4000) {
		*(.data.sce_process_param .data.sce_process_param.*)
	}

	.data.sce_module_param : ALIGN(0x4000) {
		*(.data.sce_module_param .data.sce_module_param.*)
	}
